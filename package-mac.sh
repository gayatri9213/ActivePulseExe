...
#  Builds BOTH x64 and ARM64 .dmg installers
#
...
# ── Build x64 .dmg ───────────────────────────────────────────────────
echo "[1/2] Building x64 .dmg..."
jpackage --type dmg --name "$APP_NAME-x64" --dest "$OUT_DIR" \
         --java-options "-Dactivepulse.arch=x64" \
...
jpackage --type dmg --name "$APP_NAME-ARM64" --dest "$OUT_DIR" \
         --java-options "-Dactivepulse.arch=arm64" \
